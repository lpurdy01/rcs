...
thickness = wavelength / 100 # Thickness in meters
...
# Define regions for mesh refinement
inner_region_start = -(sphere_outer_rad + (wavelength_unit / 8))
inner_region_stop = +(sphere_outer_rad + (wavelength_unit / 8))

# Maximum cell size for the coarse region (lambda/20)
max_res = C0 / f_stop / unit / 20

# Maximum cell size for the refined region (half the coarse resolution)
refined_res = max_res / 2

# Define initial mesh lines for the outer region (coarser mesh)
outer_mesh_lines = np.linspace(-SimBox / 2, SimBox / 2, int(SimBox / max_res))

# Define mesh lines for the inner region (refined mesh)
inner_mesh_lines = np.linspace(inner_region_start, inner_region_stop, int((2 * (sphere_outer_rad + wavelength_unit)) / refined_res))

# Combine the inner and outer mesh lines
mesh_lines = np.unique(np.concatenate((outer_mesh_lines, inner_mesh_lines, [-sphere_outer_rad, -sphere_inner_rad, sphere_inner_rad, sphere_outer_rad])))
mesh_lines.sort()

# Convert mesh_lines to a Python list
mesh_lines_list = mesh_lines.tolist()

# Set the initial mesh lines for the x-direction
mesh.SetLines('x', mesh_lines_list)

# Smooth the mesh lines in the x-direction
mesh.SmoothMeshLines('x', max_res)

# Retrieve the smoothed mesh lines
smoothed_lines = mesh.GetLines('x')

# Set the smoothed mesh lines for y and z directions
mesh.SetLines('y', smoothed_lines)
mesh.SetLines('z', smoothed_lines)
