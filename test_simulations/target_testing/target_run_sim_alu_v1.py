...
Sim_Path = os.path.join(tempfile.gettempdir(), 'RCS_STL_Target_Simulation_Alu')
...
PW_Box_x = 0.75  # meters
PW_Box_y = 0.75  # meters
PW_Box_z = 0.75  # meters
...
mesh_resolution = C0 / f_stop / 40  # Cell size: lambda/40 at highest frequency
...
sigma_aluminum = 3.77e7  # S/m for aluminum conductivity
aluminum_properties = {'epsilon': 1, 'kappa': sigma_aluminum}
...
polyhedron = import_stl_into_openems(
    CSX,
    stl_file_path,
    material_name='Aluminum',          # Use 'Aluminum' as the material name
    material_properties=aluminum_properties,  # Apply aluminum material properties
    priority=10,
    transform=transform
)
...
E_dir = [0, 1, 0]  # Plane wave polarization
