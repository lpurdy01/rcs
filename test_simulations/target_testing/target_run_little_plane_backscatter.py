...
stl_file_path = '../../test_targets/little_plane_asci_mesh.stl'
...
f_stop = 5000e6  # Stop frequency in Hz
...
mesh_resolution = C0 / f_stop / 20  # Cell size: lambda/40 at highest frequency
...
transform = {
    'Scale': [4, 4, 4],  # Scale from mm to meters if STL is in mm And multiply by 3x to make it larger
    'Rotate': [0.0, -90, -30],       # Rotate to have nose pointing in the +x direction
    'Translate': [0.1, -0.05, 0.0],    # Roughly center the object in the simulation box
}

# Define aluminum material properties
aluminum_properties = {
    'epsilon': 1.0,  # Relative permittivity
    'kappa': 3.77e7  # Conductivity in S/m (for aluminum)
}
...
polyhedron = import_stl_into_openems(
    ...
    material_properties=aluminum_properties,
    priority=10,
    transform=transform
)
...
## Add a electric current dump
J_dump = CSX.AddDump('J_dump', dump_type=2, file_type=0)
J_dump.AddBox(start=start, stop=stop)
